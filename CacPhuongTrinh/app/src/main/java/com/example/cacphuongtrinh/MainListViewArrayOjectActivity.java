...
import com.example.cacphuongtrinh.model.DataItem;

import java.util.ArrayList;
import java.util.List;

public class  MainListViewArrayOjectActivity extends AppCompatActivity {
    ListView lvm;
    List<DataItem> data;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_main_list_view_array_oject);
        lvm=findViewById(R.id.lvobject);
        data = new ArrayList<>();
        data.add(new DataItem("ax+b=0"));
        data.add(new DataItem("ax^2+bx+c=0"));
        data.add(new DataItem("Phép Toán"));
        data.add(new DataItem("X^n"));
        data.add(new DataItem("X^n"));
        data.add(new DataItem("X^n"));
        data.add(new DataItem("X^n"));
        data.add(new DataItem("X^n"));
        ...
        adapter = new ArrayAdapter<DataItem>(this, android.R.layout.simple_list_item_1, data);
        ...
        lvm.setOnItemLongClickListener(new AdapterView.OnItemLongClickListener() {
            ...
            public boolean onItemLongClick(AdapterView<?> parent, View view, int position, long id) {
                ...
                showDialogconfirm(data.get(position).getName(), position);
                return false;
            }
        });
